...
export const CartProvider = ({ children }) => {
  ...
  const addToCart = (productId) => {
    setCart((prevCart) => [...prevCart, parseInt(productId)]);
  };

  // Increase the quantity of a product
  const increaseQuantity = (productId) => {
    setCart((prevCart) => [...prevCart, parseInt(productId)]); // Add product to cart
  };

  // Decrease the quantity of a product
  const decreaseQuantity = (productId) => {
    setCart((prevCart) => {
      const index = prevCart.indexOf(parseInt(productId)); // Ensure comparison uses integers
      if (index !== -1) {
